...
EMOJIS = [':bowtie:', ':smile:', ':laughing:', ':blush:', ':smiley:', ':relaxed:', ':smirk:',
          ':heart_eyes:', ':kissing_heart:', ':kissing_closed_eyes:', ':flushed:', ':relieved:',
          ...
          ':cry:', ':sob:', ':joy:', ':astonished:', ':scream:', ':neckbeard:', ':tired_face:',
          ':angry:', ':rage:', ':triumph:', ':sleepy:', ':yum:', ':mask:', ':sunglasses:',
          ':dizzy_face:', ':imp:', ':smiling_imp:', ':neutral_face:', ':no_mouth:', ':innocent:', ':alien:',
          ':yellow_heart:', ':blue_heart:', ':purple_heart:', ':heart:', ':green_heart:', ':broken_heart:', ':heartbeat:',
...
class MarcusRenderer(mistune.Renderer):
    ...
    def emoji(self, text):
        ...
        :param text: emoji text to render (like ':smile:')
        """
        if text not in EMOJIS:
            return text
        emoji = text.replace(':', '')
        url = 'https://github.githubassets.com/images/icons/emoji/%s.png' % emoji
        html = '<img src="%s" alt="%s" class="marked-emoji" />' % (url, emoji)
        return html
...
class EmojiInlineLexer(mistune.InlineLexer):
    def enable_emoji(self):
        # Add emoji rules
        self.rules.emoji = re.compile(EMOJI_RE)
        # Add emoji text parser to default rules
...
def render(value, renderer=MarcusRenderer()):
    inline = EmojiInlineLexer(renderer)
    inline.enable_emoji()
